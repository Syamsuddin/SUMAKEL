        ...
        <form action="{{ route('surat-masuk.update', $suratMasuk) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="nomor_surat" id="nomor_surat" class="form-control @error('nomor_surat') is-invalid @enderror" value="{{ old('nomor_surat', $suratMasuk->nomor_surat) }}" required>
                    @error('nomor_surat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label for="asal_surat" class="form-label">Asal Surat</label>
                    <input type="text" name="asal_surat" id="asal_surat" class="form-control @error('asal_surat') is-invalid @enderror" value="{{ old('asal_surat', $suratMasuk->asal_surat) }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="klasifikasi_id" id="klasifikasi_id" class="form-select @error('klasifikasi_id') is-invalid @enderror" required>
                        @foreach($klasifikasis as $k)
                            <option value="{{ $k->id }}" {{ old('klasifikasi_id', $suratMasuk->klasifikasi_id) == $k->id ? 'selected' : '' }}>{{ $k->kode }} - {{ $k->nama }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="perihal" id="perihal" class="form-control @error('perihal') is-invalid @enderror" value="{{ old('perihal', $suratMasuk->perihal) }}" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="sifat" id="sifat" class="form-select @error('sifat') is-invalid @enderror" required>
                        @foreach(['biasa','penting','rahasia'] as $s)
                            <option value="{{ $s }}" {{ old('sifat', $suratMasuk->sifat) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                    @error('sifat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-8 mb-3">
                    <label for="lampirans" class="form-label">Tambah Lampiran</label>
